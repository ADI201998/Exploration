...
1. The class takes in an extra parameters : 
  i. level_id - an id for each level. This is used to change the frame name of the map frame for each level. This is done in the function setParams()(line 48).
                this is also used to change the topic for the map. Each map topic will have a frame_id with the level_id in it. (line 354)
2. The class publishes two static transforms instead of one. map->map_0 and map_0->odom. The map 0 corresponds to the map for the 0th level.
    In case of a change in level, the 0 will change to another value. In function publishVisualizations()(line 70)
*/

#include "slam_toolbox/slam_toolbox_custom.hpp"
...
/*****************************************************************************/
SlamToolbox::SlamToolbox(ros::NodeHandle& nh, int level_id)
...
/*****************************************************************************/
{
  ...
  //private_nh.param("map_frame", map_frame_, std::string("map_"+std::to_string(this->level_id)));
  map_frame_ = std::string("map_"+std::to_string(this->level_id));
  private_nh.param("base_frame", base_frame_, std::string("base_footprint"));
  private_nh.param("resolution", resolution_, 0.05);
  private_nh.param("map_name", map_name_, std::string("/level_"+std::to_string(this->level_id)+"/map"));
...
/*****************************************************************************/
{
  ...
  ssMap_ = node.advertiseService("/level_"+std::to_string(this->level_id)+"dynamic_map", &SlamToolbox::mapCallback, this);
  ssPauseMeasurements_ = node.advertiseService("/level_"+std::to_string(this->level_id)+"pause_new_measurements", &SlamToolbox::pauseNewMeasurementsCallback, this);
  ssSerialize_ = node.advertiseService("/level_"+std::to_string(this->level_id)+"serialize_map", &SlamToolbox::serializePoseGraphCallback, this);
  ssDesserialize_ = node.advertiseService("/level_"+std::to_string(this->level_id)+"deserialize_map", &SlamToolbox::deserializePoseGraphCallback, this);
...
/*****************************************************************************/
{
  ...
  while(ros::ok())
  {
    ...
    }
    {
      ...
      msg.transform.translation.z = 3*this->level_id;
...
/*****************************************************************************/
{
  ...
  if(scan->header.frame_id[0]=='/')
    scan->header.frame_id.erase(scan->header.frame_id.begin()+0);
  //std::cout<<scan->header.frame_id<<std::endl;
...
/*****************************************************************************/
{
  ...
  //std::cout<<"size = "<<map_.map.data.size()<<"\n";
  //std::cout<<"height = "<<map_.map.info.height<<"\n";
  //std::cout<<"width = "<<map_.map.info.width<<"\n";
  // publish map as current
  map_.map.header.stamp = ros::Time::now();
  map_.map.header.frame_id = std::string("map_")+ std::to_string(this->level_id);
...
/*****************************************************************************/
{
  //std::cout<<"INSIDE ADD_SCAN FIRST\n";
...
/*****************************************************************************/
{
  std::cout<<"Pause New Measurements Callback\n";
...
/*****************************************************************************/
{
  std::cout<<"Serialize Pose Graph Callback\n";
...
/*****************************************************************************/
{
  std::cout<<"Deserialize Pose Graph\n";
